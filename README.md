# Nocalhost Dev Container

List of dev containers:

```
codingcorp-docker.pkg.coding.net/nocalhost/dev-images/node:latest
codingcorp-docker.pkg.coding.net/nocalhost/dev-images/node:14

codingcorp-docker.pkg.coding.net/nocalhost/dev-images/golang:latest
codingcorp-docker.pkg.coding.net/nocalhost/dev-images/golang:1.16

codingcorp-docker.pkg.coding.net/nocalhost/dev-images/python:latest
codingcorp-docker.pkg.coding.net/nocalhost/dev-images/python:3.9

codingcorp-docker.pkg.coding.net/nocalhost/dev-images/java:latest
codingcorp-docker.pkg.coding.net/nocalhost/dev-images/java:11
```